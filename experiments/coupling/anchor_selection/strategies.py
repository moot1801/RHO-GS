...
from ...types import AnchorSet, RenderState
...
def _geometry_eligible(gaussian_state: Any, render_state: RenderState, config: dict[str, Any]) -> torch.Tensor:
...
class RandomEligible:
    ...
    def select(self, gaussian_state: Any, render_state: RenderState, view: Any, config: dict[str, Any], contribution_scores: torch.Tensor | None = None) -> AnchorSet:
        del view, contribution_scores
        candidates = torch.nonzero(_geometry_eligible(gaussian_state, render_state, config), as_tuple=False).flatten()
        selected = _random_subset(candidates, _take_count(config, candidates.numel()), int(config.get("seed", 0)))
        return _anchor_set(self.name, selected, candidates, config, None, {"score_basis": "none", "candidate_basis": "geometry_visible"})
...
class FixedAnchorSelection:
    ...
    def select(self, gaussian_state: Any, render_state: RenderState, view: Any, config: dict[str, Any], contribution_scores: torch.Tensor | None = None) -> AnchorSet:
        ...
        geometry_mask = _geometry_eligible(gaussian_state, render_state, config)
        ...
        return _anchor_set(self.name, selected, candidates, config, contribution_scores, {"score_basis": "provided_or_zero", "candidate_basis": "geometry_visible"})
...
class _ContributorBase:
    ...
    def select(self, gaussian_state: Any, render_state: RenderState, view: Any, config: dict[str, Any], contribution_scores: torch.Tensor | None = None) -> AnchorSet:
        ...
        eligible = _geometry_eligible(gaussian_state, render_state, config)
